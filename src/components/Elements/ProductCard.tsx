...
export const ProductCard = ({ product }: ProductCardProps) => {
  ...
  useEffect(() => {
    const productInCart = cartList.find((cartItem: Product) => cartItem.id === product.id);
  ...
  return (
    <div className="m-3 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
      <Link to={`/products/${id}`} className="relative">
        {best_seller && (
          <span className="absolute top-4 left-2 px-2 bg-orange-500 bg-opacity-90 text-white rounded whitespace-nowrap z-10">
            Best Seller
          </span>
        )}
